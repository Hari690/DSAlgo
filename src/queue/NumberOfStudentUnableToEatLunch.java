...
public class NumberOfStudentUnableToEatLunch {
    public int countStudents(int[] students, int[] sandwiches) {
        ...
        int reset=0,top=0;
        while(!studentsQueue.isEmpty() && reset<studentsQueue.size()) {
            int student = studentsQueue.poll();
            if(sandwiches[top]==student) {
                reset=0;
                top++;
            }
            else {
    ...
    public static void main(String[] args) {
        NumberOfStudentUnableToEatLunch solution = new NumberOfStudentUnableToEatLunch();
